...
二相提交两个步骤
1. 应用向所有节点询问是否准备完毕。
2. 如果所有节点准备完毕则应用发送commit请求。否则发送退出命令。

二相提交有两个关键节点：
1. 节点返回同意写，则它之后不能用某种理由拒绝或者失败。
2. 应用发送确认，或者退出。这个决定无法修改。

#### 协调员错误

如果协调员出现错误没有发送，最后确认信息，节点就会一直等待处于一种疑惑`in doubt`状态。节点之间可以互相通信确认目前状态，但是这不是2PC协议的一部分了。

#### 三相提交
为了解决协调员错误可以使用三相提交，但是三相提交必须有完美无延迟的网络才能实现原子提交。所以实际上最常用的算法依然是二相提交。

### 分布式事物处理实战
分布式事物处理特别是二相提交，会很大影响效率（10倍单机事物的时间）。所以它的应用颇具争议。有两种不同类型的分布式事物处理。

- 数据库内分布式事物：内部都是等价节点。
- 异质分布式事物：节点与节点之间不等价。

#### 每条信息只处理一次
异质系统的典型：消息队列中的数据只会被提交一次。这就需要不同的系统使用相同的分布式事物协议（比如二相提交）。不然可能会导致数据丢失或者数据重复提交的问题。

#### XA事物
这是一种CAPI广泛采用在各种分布式数据库和消息队列中。

#### 疑惑状态时的锁
如果没有收到协调员的信息，节点需要锁住事物相关的数据，防止脏写入。

#### 从协调员错误中恢复
如果协调员出现错误重启，可能会导致无法决定一些事物是否应该提交或者失败。这样可能导致节点永远被锁。此时唯一解决途径就是人工干预。很多XA的应用拥有一套启发式可能打破原子提交的干涉工具。

#### 分布式事物处理的局限性
XA解决了让多个数据系统一致的问题，但是协调员是它的弱点，很多系统的协调员不是高可用的。
- 如果协调员是单机则会导致单点失败引起整个系统挂掉。
- 协调员日志变成重要部分，因为它要用来帮助恢复系统。整个系统变成一个有状态系统。
- XA没法发现死锁。没法和SSI合作。
- 任何一个节点错误都会导致写入失败，这会使得整个系统的失败几率被放大。

### 容错共识
一个共识算法必须满足：
1. Uniform agreement: 单一性
    投票只会投出一个值。
2. Integrity: 完整性
    每个节点只会投票一次。
3. Validity: 有效性
...
前两者是决定性因素。大家会投票决定最后的结果，一旦决定无法反悔。第三点保证投票出来的值有效。第四点保证可用性,否则可以用独裁者，或者什么都不做来保证前三点。前三条保证安全，第四条保证存活。

#### 仲裁人数
所有共识算法都有leader，但是他们不保证`leader`独一无二。算法有一个`epoch`选票号码，用来决定冲突时解决顺序。`leader`本身为了保证自己的合法性需要向所有节点发送提案确认自己`leader`位置的请求。只有当请求结果超过仲裁人数时选举结果才能得到确认。选举结果的有效性需要有一个节点既参加了选举，又参加了提案来保证。需要看看<500lines cluster> 当时看不懂里面的ballot number ，这里看不到具体过程。

#### 共识的局限性
共识算法可以提供各种安全保障，提供容错的原子操作，提供完全次序广播等等。不过共识算法的成本非常高，每次决定需要多数节点来做决定。在地域隔离的分布式系统中，因为延迟会导致频繁的`leader`选举从而带来可怕的性能损失。

### 成员和协调员服务
`ZooKeeper`和`ETCD`使用了共识算法，每个节点都备份了完整的数据。并且使用完全次序广播。还使用了：
- 线性原子操作,同时全局只能有一个操作进行。
- 完全顺序操作，每个操作的时间都能互相比较。
- 错误检测，心跳检测，检测服务是否存活。
- 变更标记，可以标记出每一次变更。
...
#### 给节点配置任务
ZK的职责有`leader`选举,资源配置(弹性分配资源在不同机器上),ZK不会部署在大集群的每一台服务器上。它擅长处理长时间不变的数据而不是每秒都在改变的东西。

#### 服务发现
ZK和etcd还能进行服务发现，新服务器将自己注册到zk好让集群中已经存在的机器知道它的存在。不过这项服务并不依赖于共识。
