...
#### 从同一份事件日志中构建不同视图
这个方法和同一个stream有多个消费者一样。新增和删除一个消费者不会影响其他消费者。这样你就可以将数据的存储和读取所需的缓存分离开来。存储只用写log，那些复杂的读取缓存业务逻辑则通过不同消费者来构建。这个方法叫(command query responsibility segregation) (CQRS)。

#### 并行控制
这个方法最大的问题在于，消费者是异步的。为了解决这个问题，要么同步一些最重要的视图和日志，要么对每个用户启动一个单独的事物，或者将日志分块存储，这样每个用户只会读写同一份日志。

#### 不变性的局限性
不过只有很少发生变化的数据才适合不变日志系统，那些经常要经历变化的数据更适合可变的数据库。对那些变化频繁的数据，使用不变日志，可能导致数据过于庞大，垃圾清理回收变得困难。
...
本节介绍第三种方法。流和之前map reduce 还有文件处理有一个巨大的不同，它永远不会结束。所以排序，以及排序相关的所有算法都无法实施。容错也必须做出改变，

### 流的使用

#### 复杂事件处理
CEP（Complex event processing) 类似正则表达式。这种系统类似数据库的反向操作，数据变化，查询不变。

#### 流分析
不同于CEP分析更多会关注统计和事件聚合。比如在时间窗口中使用概率算法，很多开源流处理软件都支持这种特性，比如Spark streaming, Flink, Kafka等等。
...
### 时间的原因
很多流处理使用本地时间作为事件发生时间，但是这可能导致事件发生时间显著晚于实际时间。

#### 事件时间和处理时间
更严重的是这种时间上的不一致可能导致先后发生的事件时间顺序错乱。

#### 知道何时准备好
流处理我们可能不会知道什么时候事件会停止。这时候需要我们做出选择：
1. 不再处理之后数据。
2. 根据之后数据做出修正。

#### 使用哪个时间
用户设备时间不一定可信，所以经常需要使用三个时间来做矫正，用户设备事件发生时间，用户设备发送时间，服务器收到事件时间。

#### 窗口的类型

1. 滚动窗口，窗口之间不重叠，时间等长，间隔离散。
2. 跳跃窗口，窗口之间重叠，时间等长，间隔离散。
3. 滑动窗口，窗口之间重叠，时间等长，间隔连续。
4. 对话窗口，时间不等长，以用户为维度聚集。

### 流的join操作。
三种不同情况需要分别讨论。

#### 流与流之间的join
通过流与流相同的session id 进行join。

#### 流与table之间的join
流与一张固定表进行join。与流和流join不同的是，table的内容是会变化的。

#### table与table之间的join
只有数据量庞大时，才需要用流的方法来维护。

#### 时间依赖join
有时候join的表过去与现在状态不同，这时候需要（slowly changing dimension)_用一个版本号来区分需要join的不同版本。

### 容错
对于流系统，这需要让数据在重跑时也只处理一次。

#### 小批量和记录点
一种方法是参考批处理，将流划分为小块比如spark streaming和记录点 flink。这种方法对流内部数据有效，不过对写入数据库等的有外部交互的情况就不起作用了。

#### 原子提交
要保证外部的(exactly-once semantics)，可以使用原子提交。

#### 幂等性
幂等性是指的一个操作，实施n次的效和一次一样。比如+1不是一个幂等操作*1是一个幂等操作。storm就使用了这种方法。

#### 灾后重建
一般可以采用本地或者远程备份，对于小批量数据则可以直接重跑
